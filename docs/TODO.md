...
## Pending — engineering
- [ ] **F&O options — complete live paper-trading slice**: use FYERS (or Upstox) live market-data WebSocket for underlying + option-contract ticks; add option-chain/contract metadata (expiry, strike, CE/PE, lot size), Greeks, bid/ask spread and liquidity freshness checks, margin/max-loss validation, option-specific paper fills/costs/P&L, and tests. Never call live order placement from sandbox/paper mode.
