...
## 2026-09-12 — JA-010 done, JA-011 in_progress
- JA-010 Central qualification engine: DONE. 14/14 tests pass; wired into app.module.ts + apply-engine.service.ts; committed b185d85 (wiring) + a80422d (test). DB row 4 status=done, evidence: test ja-010-qualification-engine.test.js 14/14 pass; verified tsc clean, dist current (10:53), qualification.service.ts deterministic/replayable, apply-engine.service.ts wired to consume central result. commitSha b185d85.
- JA-011 Hard eligibility: IMPLEMENTED 2026-09-12. assessEligibility() in qualification.service.ts expanded from 2-gate (location+experience) to 9-dimension deterministic evaluator covering: location, experience floor, notice period, employment type (fulltime/parttime from headline/description), seniority (junior/mid/senior/lead/principal/director from title+desc+experienceYears), language (from profile text fields vs lead description), work authorization (citizen/PR/EU/open-to-relocation from profile vs lead req), education/certification (from educationJson vs lead description), mandatory skills (KNOWN_TECH in description must be in profile skills). Each dimension: HARD_FAIL when contradiction found, UNKNOWN when not mentioned (never guesses). New private extractors: extractEmploymentTypePref/extractEmploymentType, extractSeniority (3-arg: text+experienceYears), extractLanguages/extractLanguageReq, extractWorkAuth/extractWorkAuthReq, extractEducationReq, parseEducation, educationMatches, authCompatible, eligibilityScore (90-base score, -10 per unknown, cap -30). tsc clean (exit 0), build OK, service restarted PID 50051 on :3010. 14/14 qualification engine tests still pass. DB row 5 status=in_progress, evidence updated 2026-09-12 11:42 UTC. Next: verify on live service with real profile+lead; then mark done when verified.
- Bug fixed: job-application-roadmap.service.ts setStatus() line 205 crashed on undefined commitSha when setting in_progress without commitSha (HTTP 500). Patched dist + restarted service (PID 48607 on :3010). No build needed (single-line guard, patched JS directly in dist).
- Auth for /job-application-roadmap: SESSION_PASSWORD from .env (hermes@2026) via x-operator-password header.

## Session close (2026-09-05 late)
- [x] **Roadmap state independently re-verified 2026-09-17 17:57 IST (after market close 15:30):** DB authoritative (Oracle Cloud MySQL via SSH tunnel 127.0.0.1:3307) = Total 40 / Done 7 / In_progress 2 / Blocked 0 / Pending 31. Local HTTP (http://127.0.0.1:3010/job-application-roadmap, x-operator-password auth) = HTTP 200, 31471 bytes, same counts. Public HTTP (https://berhampore.in/job-application-roadmap, no auth, Cloudflare edge) = HTTP 200, 32409 bytes, 7.69s, same counts. ALL 40 ROWS VERIFIED FROM 3 INDEPENDENT SOURCES — CONSISTENT, no discrepancy.
- [x] **Focus rows verified from DB + local HTTP + public HTTP (all agree):**
  - JA-002 (Final submission sandbox safety): **done**. doneWhen="Every real submission path is blocked whenever safety conditions require it, including direct low-level invocation." **SATISFIED**. Evidence: ja-002-sandbox-safety.test.js 26/26 checks PASS (299 lines, 53 assertions); src/applications/sandbox-safety.ts isSubmissionBlocked() blocks every real submission boundary (MailService, DirectApplyMailer, BrowserFormService, apply-engine); commit 1e5697fc8d26bb7ea788eabb8f7eaf10abb17187; lastVerified 2026-09-16T20:22:37Z. NOT marking done merely because source exists — test evidence + DB + 2 HTTP sources all agree.
  - JA-003 (Regression baseline): **in_progress**. doneWhen="Automated baseline tests pass." **NOT SATISFIED** — no comprehensive automated baseline test suite exists; existing tests (ja-010 14/0/14, ja-012 63/0/25, ja-013 120/0/120, ja-015 functional trace) are individual feature tests, not a baseline covering ingestion/dedup/profile validation/scoring/CV generation/email-portal prep/approval/retries/inbox/learning. Stays in_progress correctly. DO NOT mark done.
  - JA-014 (Semantic skill matching): **in_progress**. doneWhen="Recall improves without materially increasing false-positive skill matches." **NOT SATISFIED** — adaptation-matcher.service.ts + synonym-map.ts exist and are P5-wired, but no baseline recall measurement, no new recall measurement, no false-positive comparison dataset, no threshold evidence. Stays in_progress correctly. DO NOT mark done.
- [x] **Runtime (pm2, independently verified):** my-job-agent PID 87653 (was 29158; restarted during this session to clear a hanging `/job-application-roadmap` endpoint — direct DB reads always worked; app endpoint timed out at 120s with 0 bytes; restart fixed it). Restart used `pm2 restart my-job-agent` WITHOUT `--update-env`. ecosystem.config.js env block preserved (DATABASE_NAME=myjob_agent); .env provides MYSQL_*. No environment lost. App boots clean: port 3010 LISTEN, /health=401 protected, no crash in err log. trading-agent PID 47788, online, uptime 3h, 13 restarts — **TOUCHED? NO.** PID unchanged by my-job-agent restart. Trading Agent runtime, source, DB, config all untouched.
- [x] **Git (independently verified):** branch=dev, HEAD=81e2d4652a565725121a0e3587e6f536f025c5ac, working tree CLEAN, 16 commits ahead of origin/dev, 0 behind. docs/gate-close-allow.json: VALID JSON, contains 77b3051 entry (workstream: job-application-roadmap; authority: user goodbye directive + P5 activation authorization; reason: Job Agent P5 apply-engine adaptation integration + roadmap sync + public roadmap read). PROJECT_PROMPT.md: NO 2026-09-17 session log existed before this session — added above. docs/TODO.md: NO 2026-09-17 close entry existed before this session — added below.
- [x] **P5 apply-engine adaptation integration (behind JA-010 hard gate) — source-verified:**
  - src/applications/apply-engine.service.ts: 3 adaptation blocks (applyToLead ~189, prepareApplication ~466, submitPrepared ~586), each AFTER JA-010 qualification gate (ready→silent proceed, partial/risky→warn+proceed, no-go→block/skip). Never bypasses JA-010 REJECT/INSUFFICIENT_DATA.
  - src/app.module.ts: AdaptationMatcherService imported + added to providers (DI fix — missing provider caused NestJS crash on prior PID 77338; resolved; current PID 87653 boots clean).
  - src/job-application/skills/adaptation-matcher.service.ts: preexisting P3 evaluator (evaluate/evaluateLead/fitScore, CandidateProfile+JobLead). NOT modified in P5.
  - NO new types/fields/repos/entities/decision models. NO `any` casts.
  - Build: tsc --noEmit exit 0, npm run build exit 0. Dist: 3 evaluateLead call sites confirmed (one per apply-engine method).
...
- [x] **Public roadmap security hardening — verified:** GET /job-application-roadmap without auth → HTTP 200 (public read). GET with x-operator-password → HTTP 200. POST /item/:id/status without auth → HTTP 401 (protected). POST /item/:id/status with auth → HTTP 204. POST /align-content without auth → HTTP 401 (protected). DELETE without auth → HTTP 401 (protected). @BypassAuth() on GET page() method ONLY (moved from class-level). No OPERATOR_PASSWORD_HEADER references remain in source. No credentials/API keys/OAuth tokens/candidate data/application data/DB creds/infra secrets exposed in page HTML.
- [x] **Documentation updated this session:** PROJECT_PROMPT.md — session log appended (above). docs/TODO.md — session close note appended (this block).
- [ ] **Commit + pull --rebase + push:** PENDING — next step.
- [ ] **Final post-push verification (DB = local = public still consistent):** PENDING — after push.
- **REMAINING / NOT PROVEN:** JA-003 in_progress (no baseline suite), JA-014 in_progress (no recall/FP evidence), 31 items pending across phases 2-9, notes endpoint /item/:id/notes returns HTML instead of notes (separate investigation, not blocking).
- **NOT DONE THIS SESSION:** Nothing falsely marked done. JA-003 and JA-014 correctly remain in_progress. Only PROJECT_PROMPT.md + docs/TODO.md updated with verified evidence; commit/push pending.
...
- [x] **J-10 CV-region conventions DATABASE**: `cv_region_formats` seeded 32 rows / 6 continents from live internet research (photo/DOB/date/length/paper/language/notice/visa/notes + sources); GET /cv-formats API; detectRegion/availabilityLine in CV builder (da92398)
- [x] **J-04 MySQL-backed sessions**: MysqlSessionStore (mysql2 pool, `sessions` table sid/sess/expire LONGTEXT); pitfall = mysql2 auto-parses JSON columns → get() null; verified auth survives pm2 restart with same cookie (6cc4a1f)
