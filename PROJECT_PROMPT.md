...
## Progress (2026-09-01)
- **Mail + pre-apply-page session (2026-09-02 evening)** — APP_SECRET had been silently DROPPED by the auth-rework .env replacement (bd420a5/749bcaf) → mail_accounts passwords undecryptable ("was working properly" report). Restored the original 48-char APP_SECRET from old git history (cffdae3/c444748/2700d41/9b21ffe) into .env, committed `2233169`. Then found swarna.s.jobs@gmail.com's DB row still held the OLD app password (Gmail rejects it — user had regenerated) → re-encrypted that ONE row with the NEW app password under the restored APP_SECRET (proper PKCS7, app-scheme, verified: node-decrypt match + real IMAP login Success + live poll endpoint HTTP 201, no more "Command failed"). bapay.9 row was already fine. Pre-apply-page "not showing JD/email" was two stacked bugs: (1) in-memory sessions wiped by pm2 restarts → logged-out browser got raw 401 JSON; fixed with `UnauthorizedHtmlRedirectFilter` (HTML-Accept → redirect to login shell; API keeps 401), commit `24bb879`; (2) card() never rendered job_leads.description → added collapsible JD block per card (verified 37/37 on live page). Both pushed to origin/dev.
...
0. **ZEBRA TECHIES APPLY — user APPROVED "OK SN APPLY TO THIS JOB" (2026-09-02 evening), see docs/TODO.md top entry** — https://zebratechiessolution.com/public/career/job-opportunities/9#apply. Laravel Developer (3–5y) + AI, Sector V Kolkata walk-in. Job NOT in job_leads yet; source has no portal adapter → needs lead create + (source adapter or direct-channel handling), then pre-apply prepare → user review → approve → muhurta send. Evidence email hr@zebratechies.com in JD. Verify stale walk-in date (22 Oct–7 Nov 2025) before send. Do NOT blind-send.
