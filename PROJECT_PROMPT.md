...
- **Operator-password auth model (2026-09-02)** — Google OAuth removed entirely. localhost = no auth; public access = single operator password (`SESSION_PASSWORD` in `.env`). Endpoints: `POST /auth/login`, `GET /auth/me`, `POST /auth/logout`, `POST /auth/change-password` (old+new, persists `.env`), `POST /auth/forgot-password` (mails password to bapay.9@gmail.com via SMTP env creds). Global `ConditionalAuthGuard` (HOST-based localhost bypass — Cloudflare tunnel arrives as loopback), `@BypassAuth`/`@AllowIps` for callbacks/webhooks/server-to-server. Root `/` serves dashboard.html (localhost: no wall; remote: password form). Auth routes rate-limited; `trust proxy` set.
