...
0. **After 09:21 IST on 2026-09-11 — inspect the pre-open capture evidence and sync the Gate 2 rows**: automated by `gate2-preopen-evidence-review` (023aa89a1b18, 09:30 IST; remove it with the cron tool if driving manually and resume this console after 09:21). Next trading window after today is Tue 2026-09-15 (Sat/Sun closed, Mon 2026-09-14 = Ganesh Chaturthi). The Upstox token dies 03:30 IST daily, so a fresh one is needed each trading morning (console paste via `scripts/upstox-token-intake.js`, or the desk's GET THE TOKEN click) before the 08:57 capture job. The first real operator OAuth click is STILL the only thing that closes row 876's gap.
