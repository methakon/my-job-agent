...
- **Envelope model (2026-09-04, user directive)** — keep ₹5,000 base, stored in the DB; after every close the stored ceiling auto-grows/shrinks by net P&L (`ceiling = max(0, capital + netPnl)`, i.e. profit − charges); `generateSignals` HOLDs an underlying whose ATM 1-lot outlay (`premium × lotSize × FNO_PAPER_QTY`) exceeds headroom (`capital + netPnl − deployed`) so a cheaper chain (SENSEX etc.) can signal instead — NIFTY ATM 26SEP is ₹13–26k/lot so it currently holds; the desk opens what actually fits.
