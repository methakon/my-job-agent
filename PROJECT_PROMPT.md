...
- Deployed with `TICK_INTERPRETER_MODE=shadow` (default): both live adapters now feed the RAW provider payload to `observe()` (FYERS `onMessage`, Upstox chain leg), which interprets and measures but never writes and never throws, so desk behaviour is unchanged until the mode is switched to `on` after live verification at the next session.

**Outstanding / decisions needed**
- Next session (market hours): prove the clean baseline, run the bounded outage → failover → recover → controlled-failback demo, then read the canonical shadow metrics (accepted/rejected/latency vs budget) on real FYERS+Upstox payloads before switching `TICK_INTERPRETER_MODE=on` and moving the desks' reads onto the canonical representation.
- **Roadmap coverage**: the canonical-interpreter + Zerodha workstream may deserve its own row (it is broader than row 877's common-feed scope). NOT created — rows are operator-only; flagged and awaiting the operator's decision.
- Raw-payload archival needs a new column (`rawPayload`/`rawHash`) if the operator wants raw bytes persisted; today raw is preserved in the interpreter result (`raw` + `rawPayloadHash`) and the hash/id travel in the stored `depth` payload.
- Row **877** stays `in_progress` (evidence + gap synced, render-verified); FYERS token still expires 06:00 IST daily.
...
0. **Common-feed workstream (row 877 = GATE 0 #10) — restore single-owner and demonstrate controlled failback.** The authorised lease-isolation fix (`c2d369a`) is DONE and LIVE: the arbiter's lease read/write runs on its own dedicated MySQL connection, and the agent's `FYERS_WS` heartbeat now renews every 15 s (it had been frozen 647 s) — `test:lease-connection` 6/6, pool size untouched, 8 s bound kept, TTL rules unchanged. Baseline element "fresh FYERS heartbeat/lease" is PROVEN; "FYERS owns NIFTY / is publishing NIFTY / Upstox standby" needs market hours (market closed before the afternoon deploy), so at the NEXT SESSION: (1) prove the clean baseline, (2) run the bounded 100 s FYERS outage → UPSTOX_REST NIFTY failover → FYERS recovery → controlled failback demo with lease+publication+common-store evidence at each stage, (3) read the canonical-interpreter shadow metrics (`TICK_INTERPRETER_MODE=shadow`, wired into both live adapters) and, if clean, switch to `on` and move the desks' reads onto the canonical representation. Also pending: a roadmap-coverage decision for the canonical-interpreter/Zerodha workstream (flagged to the operator, no row invented) and a decision on persisting raw payload bytes (needs a new column). Do not mark row 877 done until the full doneWhen holds.
