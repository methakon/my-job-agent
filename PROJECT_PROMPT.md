...
- **Google OAuth login wired (2026-09-01)** — `.env` carries real `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALLBACK_URL=https://berhampore.in/auth/google/callback`, `GOOGLE_ALLOWED_EMAILS` (bapay.9@gmail.com + swarna.s.jobs@gmail.com). `src/auth/` = Passport Google strategy + auth.guard localhost bypass (localhost needs no registered callback). Node 26 passport import fixed (`import passport from 'passport'`, esModuleInterop). App healthy on 3010 after `pm2 restart --update-env`. **User action pending:** Google Cloud Console — consent screen External + test users + register the single redirect URI. Then E2E test.
...
0. **Google OAuth login — finish E2E** (2026-09-01): user registers the single Authorized redirect URI `https://berhampore.in/auth/google/callback` + consent screen (External, test users bapay.9@gmail.com + swarna.s.jobs@gmail.com) in Google Cloud Console — user-side. Then test login at `https://berhampore.in/auth/google`.
