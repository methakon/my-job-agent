...
0. **Common-feed workstream (row 877 = GATE 0 #10) — restore single-owner and demonstrate controlled failback.** The authorised MySQL pool reliability fix is DONE, tested and deployed (`658cbef`: keepalive + bounded idle reaping, live-proven — an idle socket is reaped 30 s after it goes idle and the next query opens a fresh connection in 523 ms), but it did NOT cure the frozen FYERS lease: after a clean restart the agent's `FYERS_WS` heartbeat wrote once (13:32:42) and has not advanced since, its lease read timing out at the 8 s bound on every poll (logged at 13:32:07, 13:37:07 — one line per 5-min throttle window). Evidence now narrows the cause: NOT stale sockets, NOT the DB (independent `SELECT 1` 167-280 ms, no lock waits, `Threads_running` 5), NOT the code path (the same compiled `FeedArbitrationService` + real DataSource does `find()` in 200-283 ms and `beat()` in 391-562 ms in a fresh process, probe row cleaned up), NOT client queue starvation of the tick path (the agent's own quote inserts keep landing, ~47 rows/s, `createdAt` fresh). So it is process-state specific inside the agent, and a second, separate defect class is evidenced: during the tunnel outage the pool was closed and never recovered in-process (`Pool is closed.` storm at 12:58:52 killing quote AND snapshot AND lease writes while the process stayed up — the agent then holds the FYERS socket with zero persistence). Because the operator's baseline gate ("fresh FYERS lease/heartbeat, FYERS owns NIFTY, FYERS ticks fresh, Upstox healthy standby") does NOT hold, the bounded 100 s outage → failover → recovery → controlled failback demo was correctly NOT run; the demo script is ready to run the moment the baseline passes. Failover itself remains PROVEN (`UPSTOX` served NIFTY during the earlier real 100 s FYERS outage). Awaiting an operator decision: (a) grant the MySQL `PROCESS` privilege so live sessions/transactions can be inspected; (b) give the arbiter lease ops a reserved/dedicated connection so they cannot queue behind the hot write path; (c) authorise in-process pool recovery (bounded re-create when the pool is closed/fatal, instead of staying dead); (d) authorise reducing/batching the agent's write load. Do not mark row 877 done until the full doneWhen holds.
