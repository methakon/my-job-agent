 ...
 *
 * Every window is computed from THIS process's clock and bound as a parameter —
 * never with the DB server's NOW() (that server's clock is hours off UTC).
 */
const fs = require('fs');
...
async function main() {
	...
	const cutoffUtc = ms.utcWall(new Date(startedAt.getTime() - windowMinutes * 60_000));
	const cutoffIst = ms.istWall(new Date(startedAt.getTime() - windowMinutes * 60_000));
	const db = await ms.pool();
	const evidence = { asOf: startedAt.toISOString(), ist: ms.istStamp(startedAt), windowMinutes, cutoffUtc, app: null, socketOwnerLog: null, store: {}, desks: {}, keys: {} };
	...
	try {
		...
		const [quoteSummary] = await db.query(
			`SELECT source, COUNT(*) rows_, COUNT(DISTINCT instrumentKey) instruments,
			        ...
			        SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(depth,'$.payloadHash')) IS NULL
			                   OR JSON_UNQUOTE(JSON_EXTRACT(depth,'$.payloadHash')) = '' THEN 1 ELSE 0 END) missing_hash
			   FROM unified_option_quotes incoming
			  WHERE createdAt > ? GROUP BY source ORDER BY rows_ DESC`, [cutoffUtc]);
		...
		const [snapshotSummary] = await db.query(
			`SELECT source, COUNT(*) rows_, GROUP_CONCAT(DISTINCT symbol ORDER BY symbol SEPARATOR ', ') symbols,
			   ...
			   FROM unified_market_snapshots WHERE createdAt > ? GROUP BY source ORDER BY rows_ DESC`, [cutoffUtc]);
		...
		const [keyAudit] = await db.query(
			`SELECT instrumentKey, source, underlying, exchange, instrumentType, optionType, strike, expiry, ltp
			   FROM unified_option_quotes WHERE createdAt > ? ORDER BY createdAt DESC LIMIT 4000`, [cutoffUtc]);
		...
		const [hashAudit] = await db.query(
			`SELECT COUNT(*) n FROM unified_option_quotes WHERE createdAt > ?
			   AND depth IS NOT NULL AND COALESCE(JSON_UNQUOTE(JSON_EXTRACT(depth,'$.payloadHash')),'') = ''`, [cutoffUtc]);
		const hashless = Number(hashAudit[0]?.n ?? 0);
		...
		check('provenance preserved on every canonical row (payload hash)', hashless === 0, `${hashless} canonical rows without depth.payloadHash`);
		for (const r of quoteSummary) if (Number(r.missing_hash ?? 0) > 0) console.log(`    (note) ${r.missing_hash} depth-less rows from ${r.source} are reported as NON-CANONICAL below`);
		...
		const [legacyRows] = await db.query(
			`SELECT source, COUNT(*) n, DATE_FORMAT(MAX(createdAt),'%H:%i:%s') newest FROM unified_option_quotes
			  WHERE createdAt > ? AND depth IS NULL GROUP BY source ORDER BY n DESC`, [cutoffUtc]);
		evidence.store.nonCanonicalWriters = legacyRows;
		for (const r of legacyRows) console.log(`  NON-CANONICAL writer [${r.source}]: ${r.n} rows, newest createdAt(UTC) ${r.newest}`);
		check('every row in the window came from the canonical pipeline (no foreign legacy writer)', legacyRows.length === 0,
			legacyRows.length ? `${legacyRows.map((r) => `${r.source}=${r.n} rows without depth (newest ${r.newest} UTC)`).join('; ')} — a pre-canonical producer is still writing` : 'all rows carry canonical depth');
		...
		const [seqRanges] = await db.query(
			`SELECT source, MIN(sequenceNumber) mn, MAX(sequenceNumber) mx, COUNT(*) n FROM unified_option_quotes
			  WHERE createdAt > ? GROUP BY source ORDER BY n DESC`, [cutoffUtc]);
		evidence.store.sequenceRanges = seqRanges;
		for (const r of seqRanges) console.log(`  producer ${r.source}: seq ${r.mn}-${r.mx} over ${r.n} rows${Number(r.mx) > 5000 ? '  <-- a counter this high means a long-lived FOREIGN producer (local processes restart at 1)' : ''}`);
		...
		const [shared] = await db.query(
			`SELECT instrumentKey, COUNT(DISTINCT source) sources, GROUP_CONCAT(DISTINCT source ORDER BY source) who,
			   ...
			   FROM unified_option_quotes WHERE createdAt > ? GROUP BY instrumentKey
			  HAVING sources > 1 ORDER BY instrumentKey LIMIT 10`, [cutoffUtc]);
		...
		const [sourceUnderlying] = await db.query(
			`SELECT source, underlying, COUNT(DISTINCT instrumentKey) instruments FROM unified_option_quotes
			  WHERE createdAt > ? GROUP BY source, underlying ORDER BY instruments DESC LIMIT 8`, [cutoffUtc]);
		...
		const [fnfProviders] = await db.query(
			`SELECT provider, COUNT(*) rows_, COUNT(DISTINCT contractSymbol) contracts
			   FROM fnf_option_quotes WHERE createdAt > ? GROUP BY provider ORDER BY rows_ DESC`, [cutoffUtc]);
		...
		const [signals] = await db.query(
			`SELECT underlying, \`signal\`, feedSource, COUNT(*) n FROM pattern_signals WHERE createdAt > ?
			  GROUP BY underlying, \`signal\`, feedSource ORDER BY n DESC LIMIT 12`, [cutoffUtc]);
		...
		for (const [label, sql] of [
			['fnf_trades', `SELECT COUNT(*) n FROM fnf_trades WHERE orderedAt > ?`],
			['fnf_decision_journal', `SELECT COUNT(*) n FROM fnf_decision_journal WHERE createdAt > ?`],
			['upstox_orders', `SELECT COUNT(*) n FROM upstox_live_paper_orders WHERE createdAt > ?`],
			['upstox_candidates', `SELECT COUNT(*) n FROM upstox_live_paper_candidates WHERE evaluatedAt > ?`],
		]) {
			try {
				const [row] = await db.query(sql, [cutoffUtc]);
